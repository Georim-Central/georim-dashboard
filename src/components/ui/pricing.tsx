...
All plans include access to our platform, lead generation tools, and dedicated support.`,
  ...
  const selectedPlan = plans.find((plan) => plan.isCurrentPlan)?.id ?? plans.find((plan) => plan.isCurrentPlan)?.name ?? null;
      ...
      <div className="mb-10 flex justify-center">
        <div className="flex flex-wrap items-center justify-center gap-3 rounded-full border border-[#7626c6]/15 bg-[#f7f1ff] px-4 py-3 shadow-sm">
          ...
          <span
            className={cn(
              'text-sm font-semibold transition-colors duration-200',
              isAnnual ? 'text-[#7626c6]' : 'text-gray-500'
            )}
          >
            Annual billing <span className={cn('transition-colors duration-200', isAnnual ? 'text-[#7626c6]' : 'text-[#7626c6]/80')}>(Save 20%)</span>
          </span>
        </div>
      </div>
          ...
          >
            <motion.div
              ...
              className={cn(
                ...
                isSelected
                  ? 'border-[#7626c6] shadow-[0_24px_60px_rgba(118,38,198,0.16)]'
                  : 'border-gray-200',
                !plan.isPopular && 'mt-4'
              )}
            >
                ...
                <ul className="mt-6 flex flex-col gap-3">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-2">
                      <Check className="h-4 w-4 text-[#7626c6] mt-1 flex-shrink-0" />
